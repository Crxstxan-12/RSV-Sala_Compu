...
export default function ChartsDashboard({ datosPorDia, datosEstados, datosPorSemana, datosRecursos }) {
  return (
    <div className="charts-grid">
      {/* Barras: reservas por día de la semana */}
      <div className="chart-card">
        <h3>Reservas por día de la semana</h3>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={datosPorDia} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
      ...
      {/* Barras: equipos solicitados */}
      <div className="chart-card">
        <h3>Equipos más solicitados</h3>
        {datosRecursos.length === 0 ? (
          <p className="chart-empty">Sin solicitudes en el período seleccionado</p>
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={datosRecursos} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="recurso" tick={{ fontSize: 10 }} />
              <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
              <Tooltip />
              <Bar dataKey="total" fill="#059669" radius={[4, 4, 0, 0]} maxBarSize={60} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
