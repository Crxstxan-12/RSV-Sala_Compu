...
export default function Informes() {
  ...
  const solicitudesFiltradas = useMemo(() =>
    solicitudes.filter(s => s.fecha_uso >= fechaInicio && s.fecha_uso <= fechaFin),
  ...
  const stats = useMemo(() => {
    ...
    reservasFiltradas.forEach(r => {
      ...
      porProfesor[nombre] = (porProfesor[nombre] || 0) + 1
    })
    ...
    solicitudesFiltradas.forEach(s => {
      const recurso = s.recurso || 'Sin recurso'
      porRecurso[recurso] = (porRecurso[recurso] || 0) + 1
    })
  ...
  const datosEstados = useMemo(() => {
    ...
    return ['aprobada', 'pendiente', 'rechazada']
      .map(e => ({
        name: etiquetas[e],
        value: reservasFiltradas.filter(r => r.estado === e).length,
      }))
      .filter(d => d.value > 0)
  }, [reservasFiltradas])
  ...
  const datosPorSemana = useMemo(() => {
    ...
    return Object.entries(semanas)
      ...
      .map(([key, total]) => ({
        semana: `Sem. ${key.slice(5).replace('-', '/')}`,
        total,
      }))
  }, [reservasFiltradas])
  ...
  const datosRecursos = useMemo(() => {
    ...
    solicitudesFiltradas.forEach(s => {
      const recurso = s.recurso || 'Sin recurso'
      conteo[recurso] = (conteo[recurso] || 0) + 1
    })
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      const fechaGen = ahora.toLocaleDateString('es-CL', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
      })
      const horaGen = ahora.toLocaleTimeString('es-CL', {
        hour: '2-digit',
        minute: '2-digit',
      })
      ...
      setError(e.message || 'No se pudo generar el PDF. Inténtalo nuevamente.')
    } finally {
  ...
  return (
    <div className="app">
      <header className="header">
        <div className="header-content">
          ...
          <div className="header-user">
            ...
            <button onClick={signOut} className="btn-small btn-logout">
              Cerrar Sesión
            </button>
          </div>
        </div>
      ...
      <nav className="nav">
        ...
        <button className="nav-btn nav-btn-current" disabled>
          Informes
        </button>
      </nav>
